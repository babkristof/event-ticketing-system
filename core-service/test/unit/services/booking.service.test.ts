...
describe('Booking Service', () => {
    ...
    describe('create', () => {
        it('should create a new booking and decrement available tickets', async () => {
            ...
            expect(loggerSpy).toHaveBeenCalledWith(`Booking created successfully for user ${mockUser.id} for event ${mockEvent.id}`);
        ...
        it('should throw ConflictException if not enough tickets are available', async () => {
            ...
            await expect(bookingService.create(mockEvent.id, mockUser, mockBooking.ticketCount)).rejects.toThrow(ConflictException);
        });
    });
