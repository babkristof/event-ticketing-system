...
describe('Event Service', () => {
  ...
  describe('remove', () => {
    ...
    it('should successfully delete an event and send email notifications', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        `Event with ID ${mockEvent.id} and all associated bookings were deleted successfully`
      );
    });
    ...
    it('should log an error if email sending fails for a booking', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(`Failed to send cancellation email to user ID ${mockUser.id}`, {
        error: emailJobError
      });
    });
  });
  ...
  describe('Event Service - Update Function', () => {
    ...
    it('should log an error if email notification fails', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to send update notification email'), {
        error: emailError
      });
    });
  });
