...
describe('Event Routes Integration Tests', () => {
  ...
  describe('GET /api/events/:id', () => {
    it('should retrieve an event by ID', async () => {
      ...
      const response = await request(app).get(`/api/events/${eventId}`).set('Authorization', `Bearer ${token}`);
    ...
    it('should return 400 for an invalid event ID format', async () => {
      const response = await request(app).get('/api/events/invalid-id').set('Authorization', `Bearer ${token}`);
    ...
    it('should return 401 if a user with CUSTOMER role tries to get an event by ID', async () => {
      ...
      const response = await request(app).get(`/api/events/${eventId}`).set('Authorization', `Bearer ${userToken}`);
  ...
  describe('DELETE /api/events/:id', () => {
    it('should successfully delete an event when requested by an admin', async () => {
      ...
      const response = await request(app).delete(`/api/events/${eventId}`).set('Authorization', `Bearer ${token}`);

      expect(response.status).toBe(204);

      const getResponse = await request(app).get(`/api/events/${eventId}`).set('Authorization', `Bearer ${token}`);
    ...
    it('should return 401 if a user with CUSTOMER role tries to delete an event', async () => {
      ...
      const response = await request(app).delete(`/api/events/${eventId}`).set('Authorization', `Bearer ${userToken}`);
    ...
    it('should return 409 if attempting to delete a past event', async () => {
      ...
      const response = await request(app).delete(`/api/events/${pastEventId}`).set('Authorization', `Bearer ${token}`);
