...
describe('/me Endpoint Integration Tests', () => {
  ...
  it('should return user profile if token is valid', async () => {
    const response = await request(app).get(USER_ME_ENDPOINT).set('Authorization', `Bearer ${token}`);
  ...
  it('should return 401 if token is invalid', async () => {
    const response = await request(app).get(USER_ME_ENDPOINT).set('Authorization', 'Bearer invalidToken');
